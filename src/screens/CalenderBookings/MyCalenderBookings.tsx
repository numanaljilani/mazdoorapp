...
const MyCalenderBookings = ({navigation}: any) => {
  ...
  const [myAppointment, {error, data}] = useMutation(MYDATEAPPOINTMENT);
  ...
  return (
    <>
      <ScrollView
        contentContainerStyle={{backgroundColor: dark ? '#1F1F1F' : 'white'}}>
        ...
        <View className={bg_color(dark)}>
          ...
          <View className={`${bg_color(dark)} min-h-screen`}>
            {bookings.length > 0 ? (
              ...
              <NotFoundBooking
                desc={'No scheduled orders currently at this date.'}
              />
            )}
          </View>
